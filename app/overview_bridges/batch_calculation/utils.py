...
def deserialize_batch_results(stored_file: File) -> dict[int, dict[str, Any]]:
    ...
    from viktor.core import File
    print(f"DEBUG [deserialize_batch_results:ENTRY]: Received type={type(stored_file)}, isinstance(bool)={isinstance(stored_file, bool)}, isinstance(File)={isinstance(stored_file, File)}, value={stored_file}", flush=True)
